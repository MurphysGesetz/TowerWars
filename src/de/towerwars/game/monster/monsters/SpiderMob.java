...
public class SpiderMob extends InheritEntity {

    public SpiderMob(World world, Location location, Location targetLocation, TowerWarsTeam towerWarsTeam, TowerWarsTeam attackerTowerWarsTeam) {
        super(world);
        int live = 700;
        ((LivingEntity) this.getBukkitEntity()).setMaxHealth(live);
        this.setHealth(live);
        final float[] moveValue = randFloat();
        this.goalSelector.a(9, new PathFinderGoalAttacker(this, targetLocation, (float) 1.4, towerWarsTeam, attackerTowerWarsTeam, towerWarsTeam.getPath(), moveValue));
        this.setLocation(location.getX(), location.getY(), location.getZ() + moveValue[1], location.getYaw(), location.getPitch());
        this.setCustomName("ag" + live);
