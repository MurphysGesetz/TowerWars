...
public class SilverfishMob extends InheritEntity {

    public SilverfishMob(World world, Location location, Location targetLocation, TowerWarsTeam towerWarsTeam, TowerWarsTeam attackerTowerWarsTeam) {
        super(world);
        int live = 5;
        ((LivingEntity) this.getBukkitEntity()).setMaxHealth(live);
        this.setHealth(live);
        final float[] moveValue = randFloat();
        this.goalSelector.a(9, new PathFinderGoalAttacker(this, targetLocation, (float) 1.0, towerWarsTeam, attackerTowerWarsTeam, towerWarsTeam.getPath(), moveValue));
