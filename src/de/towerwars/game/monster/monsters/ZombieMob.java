...
import de.towerwars.game.monster.pathfinder.PathFinderGoalAttackerZombie;
...
public class ZombieMob extends InheritEntity {

    public ZombieMob(World world, Location location, Location targetLocation, TowerWarsTeam towerWarsTeam, TowerWarsTeam attackerTowerWarsTeam) {
        super(world);
        int live = 300;
        ((LivingEntity) this.getBukkitEntity()).setMaxHealth(live);
        this.setHealth(live);
        final float[] moveValue = randFloat();
        this.goalSelector.a(9, new PathFinderGoalAttackerZombie(this, targetLocation, (float) 1.2, towerWarsTeam, attackerTowerWarsTeam, towerWarsTeam.getPath(), moveValue));
        this.setLocation(location.getX(), location.getY(), location.getZ() + moveValue[1], location.getYaw(), location.getPitch());
        this.setCustomName("ae" + live);
