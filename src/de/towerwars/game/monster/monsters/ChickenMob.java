...
public class ChickenMob extends InheritEntity {

    public ChickenMob(World world, Location location, Location targetLocation, TowerWarsTeam towerWarsTeam, TowerWarsTeam attackerTowerWarsTeam) {
        super(world);
        int live = 40;
        ((LivingEntity) this.getBukkitEntity()).setMaxHealth(live);
        this.setHealth(live);
        final float[] moveValue = randFloat();
        this.goalSelector.a(9, new PathFinderGoalAttacker(this, targetLocation, (float) 1.2, towerWarsTeam, attackerTowerWarsTeam, towerWarsTeam.getPath(), moveValue));
        this.setLocation(location.getX(), location.getY(), location.getZ() + moveValue[1], location.getYaw(), location.getPitch());
        this.setCustomName("ab" + live);
